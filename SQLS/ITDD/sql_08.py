...
tmp.con_year AS '年份',
tmp.con_month AS '月份',
CASE WHEN tmp.pay_fee <100 THEN '100元以下'
   WHEN tmp.pay_fee >=100 AND tmp.pay_fee <=300 THEN '100~300元'
   WHEN tmp.pay_fee >300 THEN '300元以上'
ELSE '其他' END AS '订单金额区间',
SUM(tmp.pay_fee) AS '订单总金额',
COUNT(tmp.order_id) AS '订单总数量',
COUNT(DISTINCT tmp.buyerNick) AS '消费者（下单人ID）总数量'
FROM
(SELECT 
tbs.tradeMainOrderId AS order_id,
tbs.buyerNick AS buyernick,
YEAR(tbs.orderConsignTime) AS con_year,
MONTH(tbs.orderConsignTime) AS con_month,
SUM(((tbso.itemAmount*tbso.itemPrice) - IFNULL(tbso.itemDiscountFee,0) - IFNULL(tbso.partMjzDiscount,0)
  + IFNULL(tbso.adjustFee,0))/100 + IFNULL(tbso.taxFee,0))  AS pay_fee
...
GROUP BY tbs.tradeMainOrderId) AS tmp
GROUP BY 1,2,3;
'''
