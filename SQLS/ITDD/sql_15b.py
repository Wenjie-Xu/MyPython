'''
15 - 年度月度同一收件人退款金额、退款订单数量
'''
from SQLS.ITDD.common import date_limit

sql_01 = '''-- 15、年度月度同一收件人退款金额、退款订单数量
SELECT 
YEAR(tbs.orderConsignTime) AS '年份',
MONTH(tbs.orderConsignTime) AS '月份',
CASE WHEN IFNULL(so.recipientMobile,'')='' THEN so.recipientPhone ELSE so.recipientMobile END AS '收件手机',
COUNT(DISTINCT tbs.tradeMainOrderId) AS '退款次数',
SUM(IFNULL(tbso.refundFee,0)/100) AS '退款金额'
...
AND tbso.refundApplyTime > tbs.endTime -- 4、交易成功后申请
AND tbso.refundStatus = 'SUCCESS' -- 5、申请并退款成功
AND (so.internalRemark NOT LIKE '%试拍%' AND so.omsRemark NOT LIKE '%试拍%') -- 6、剔除试拍订单
GROUP BY 1,2,3
ORDER BY 5 DESC LIMIT 10000;
'''
