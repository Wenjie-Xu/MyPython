...
tbs.orderConsignTime AS '发货时间',
so.remark AS '顾客备注',so.omsRemark AS '系统备注',so.internalRemark AS '客服备注',
GROUP_CONCAT(DISTINCT CONCAT(p.`name`,'(价格',p.salePrice,')')) AS '订单特征',
...
AND (tbs.tradeMainOrderId IS NULL OR tbs.orderPaidTime IS NULL)
